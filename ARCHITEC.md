...
управляется простым планировщиком.
...
- `startDayNow(chatId, day)`:
  ...
  - Если день помечен `isPremium`, отправляет заглушку и не стартует.
...
- `advanceState(chatId)`:
  ...
  - Если день закончился, переходит к следующему:
    - Если следующий день `isPremium`, переводит в `scheduled` и ждет ручного запуска.
...
- `/start` — стартует день 0 и отправляет цепочку до первого feedback.
...
1. Бот инициирует оплату, создавая запись `payments` со статусом `pending`.
2. Пользователь оплачивает, ЮKassa отправляет webhook.
3. Webhook обновляет `payments.status` и, при успехе, активирует `subscriptions` или доступ к
   premium.
4. Логика `message-flow` проверяет доступ при `isPremium`.
...
- Endpoint `/webhooks/yookassa` принимает события, валидирует подпись и обновляет `payments`.
- Логика webhooks должна быть идемпотентной (по `payment_id`/`idempotency_key`).
